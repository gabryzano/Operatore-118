            ...
            setTimeout(() => {
                dettagli.querySelectorAll('.btn-conferma-trasporto').forEach(btn => {
                    btn.onclick = () => {
                        ...
                        aggiornaMissioniPerMezzo(mezzo);
                        if (window.avanzaMezzoAStato4DopoConfermato) window.avanzaMezzoAStato4DopoConfermato(mezzo);
                        ...
                        if ((mezzo.tipo_mezzo.startsWith('MSB') || mezzo.tipo_mezzo.endsWith('_A') || mezzo.tipo_mezzo === 'ELI') && mezzo._trasportoConfermato && mezzo.ospedale) {
                            (mezzi || []).forEach(m => {
                                if ((m.tipo_mezzo === 'MSA1' || m.tipo_mezzo === 'MSA2') && m._attesaDestinazioneDa === mezzo.nome_radio) {
                                    m.ospedale = mezzo.ospedale;
                                    ...
                                    if (window.avanzaMezzoAStato4DopoConfermato) window.avanzaMezzoAStato4DopoConfermato(m);
                                }
                            });
                        }
                    };
