...
<<<<<<< HEAD
                    // Cerca codice in parentesi quadre in qualunque punto, non solo alla fine
...
<<<<<<< HEAD
        // Deduplica per nome_radio per evitare doppioni di veicoli in stato 1
        const uniqueMap = {};
        const uniqueMerged = [];
        merged.forEach(m => {
            if (!uniqueMap[m.nome_radio]) {
                uniqueMap[m.nome_radio] = true;
                uniqueMerged.push(m);
            }
        });
