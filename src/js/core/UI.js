...
class GameUI {
                    ...
                    if (m.tipo_mezzo === 'MSA1' || m.tipo_mezzo === 'MSA2') {
                        ...
                        const selectOsp = `<select class='select-ospedale' data-nome='${m.nome_radio}'>`+
                            ...
                            ospedaliOrdinati.map(o => {
                                ...
                                const label = o.nome.trim();
                                return `<option value="${label}">${label} (${pct}%)${distText}</option>`;
                            }).join('')+
                            `</select>`;
    ...
    updateStatoMezzi(mezzoCambiato = null) {
        ...
        window.game.hospitals.sort((a, b) => {
            // Group: base (0), SRP (1), SRL (2), SRM (3)
            const groupKey = name => {
                if (name.startsWith('(SRP)')) return 1;
                if (name.startsWith('(SRL)')) return 2;
                if (name.startsWith('(SRM)')) return 3;
                return 0;
            };
            const ga = groupKey(a.nome || ''), gb = groupKey(b.nome || '');
            if (ga !== gb) return ga - gb;
            // Sort by descending occupancy percentage
            const capA = Number(a.raw["N° pazienti Max"] || 0);
            const capB = Number(b.raw["N° pazienti Max"] || 0);
            const cntA = window.game.hospitalPatientCount[a.nome] || 0;
            const cntB = window.game.hospitalPatientCount[b.nome] || 0;
            const pctA = capA ? cntA / capA : 0;
            const pctB = capB ? cntB / capB : 0;
            return pctB - pctA;
        }).forEach(h => {
