...
<<<<<<< HEAD
        ...
        try {
            ...
            if (!response.ok) throw new Error(`HTTP ${response.status} loading src/data/chiamate.json`);
            text = await response.text();
        } catch (e) {
            console.warn('Errore caricamento src/data/chiamate.json, riprovo con data/chiamate.json', e);
            const response2 = await fetch('data/chiamate.json');
            if (!response2.ok) throw new Error(`HTTP ${response2.status} loading data/chiamate.json`);
            text = await response2.text();
        }
        // Rimuovi eventuali commenti JS e parse JSON
